...
class sample_cxig8915(psdata.Detector):
    """Sample Control for Bergmann cxig8915 Detector Class of psdata.Detector.
    ...
    def __init__(self,  *args,**kwargs):
        ...
        self.include_detector('CsPadKalpha', module='cspad2x2')
        self.include_detector('CsPadKbeta', module='cspad2x2')

    @property
    def image(self):
        if self.cspad == 'kalpha':
            return self.CsPadKalpha.image
        else:
            return self.CsPadKbeta.image

    def ami_clear(self):
        """Clear AMI accumulation.
        """
        if self.cspad == 'kalpha':
            return self.CsPadKalpha.ami.ami_clear()
        else:
            return self.CsPadKbeta.ami.ami_clear()
    ...
    def ami_entries(self):
        ...
        """
        if self.cspad == 'kalpha':
            return self.CsPadKalpha.ami.entries
        else:
            return self.CsPadKbeta.ami.entries

    def set_kalpha(self):
        """Set configuration for kalpha.
        """
        self.set_parameter(cspad='Kalpha')

    def set_kbeta(self):
        """Set configuration for kalpha.
        """
        self.set_parameter(cspad='Kbeta')

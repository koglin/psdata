...
class Camera(psdata.Detector):
    ...
    @property
    def raw_image(self):
        """Raw image -- currently looks for an image in the following order:
           1) detector (PyDetector) data and returns the image if it 
              has a calib object.
              (only needed until next PyDetector release that should fix a bug).
           2) evtData data16 first then data8
           3) ami

           This could in the future be updated to take other types of data
           including epics.
        """
        if self.detector and self.detector.calib:

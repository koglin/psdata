...
class Scan(psdata.Detector):
    ...
    def __init__(self,  *args,**kwargs):
        """keyword args:
              picker:  pulse picker 'detector' name [default = 'picker']
            
            By convention the cxi dia pulse picker is named 'picker'
            and the picker for ssc in the ds1 chamber is 'sc3picker'.
            The picker can be accessed by this name or self._picker
        
        """

        psdata.Detector.__init__(self, *args,**kwargs)

        self._pickername = kwargs.get('picker', 'picker')
    
        self.include_detector('burst', module='burst')
        self.include_detector('sequencer', module='sequencer')
        self.include_detector(self._pickername, module='pulsepicker')
        self._picker = getattr(self, self._pickername)
        if self.daq is None:
            self._data.load_daq()
...
#                    position_dict[attr] = np.array(item['positions'])
        ...
        if sequence:
            self.sequencer.stop()
            time.sleep(0.2)
            self._picker.make_sequence(nshots, burst=burst, picker=picker, 
...
#                   reset=True,
        ...
        self.message.add(' - Start Time:  '+time.ctime())
        self.message.add('')
        #self.message.add(_ih[-1])
        self.message.add('-'*100)

        theta=tangle*pi/180.

        if self.x.RTYP == 'arcus':
            xscale = 1000.
            if trange > 0:
                reverse = False
            else:
            ...
            self.x.set_speed(xspeed)
            xvmax = self.x.VMAX
            self.message.add(' - Piezo pulse speed [pps]: {:}'.format(xvmax))
        else:
            xscale = 1.
            self.x.VELO = abs(xspeed)

        move_attrs = ['x']
        delta_dict = {'x': trange*xscale}
        self.message.add(' - sample x speed = {:} mm/s'.format(xspeed))
        if tangle:
            move_attrs.append('z')
            zdist = trange*tan(theta)*zscale
            zspeed = xspeed*tan(theta)
            delta_dict.update({'z': zdist})
            if self.z.RTYP == 'arcus':
                zscale = 1000.
                if zdist > 0:
                    reverse = False
                else:
        ...
        try:
            for istep in range(ynumsteps):
                ...
                if sequence and self.sequencer.play_status.VAL != 1:
                    self._picker.close()
                    self.sequencer.stop()
                    if shoot_rate == 120 and nshots > 1:
                        self._picker.follower_mode()

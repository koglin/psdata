...
class LiveData(object):
    """LCLS Live data class
    """
    _epics_devices = {}
    _epics_live = False
    _device_sets = {}
    _kwargs_list = ['instrument', 'platform', 'proxy_host'] 

    def __init__(self, **kwargs):
        for key in kwargs:
            if key in self._kwargs_list:
                print 'setting', key, kwargs[key]
                setattr(self, key, kwargs[key])

        self._epics_live = True
        self.ami = Ami(**kwargs)
#        self.load_epics_dict()

    def load_epics_dict(self, epics_dir=None,epics_file=None):
...
#        """
#        if self._data._epics_live:
#            if attr in self._det['pvs']:
#                pv = self._det['pvs'][attr]['base']
#            else:
#                if not pv:
#                    pv = attr.split('.')[0].replace(':','_')
#                self._det['pvs'][attr]['base'] = pv
#            self._data._epics_devices[pv] = lcls_devices.get_live_pv(pv)
#        else:
#            print 'WARNING: Epics is not live.'
#            print '         Must be on appropriate machine and _epics_live set to True.'
    ...
    def add_live_pv(self, pv):
        ...
        """
        if pv not in self._epics_devices or not self._epics_devices[pv]:
            self._epics_devices[pv] = lcls_devices.get_live_pv(pv)        
        if not self._epics_devices[pv]:
            time.sleep(0.1)
            self._epics_devices[pv] = lcls_devices.get_live_pv(pv)        
...
if __name__ == "__main__":
    ...
    if not args.instrument:
        args.instrument = args.proxy_host.split('-')[1]
    if not args.base and args.instrument:
...
#    try:
    if True:
        print "*"*80
        if args.base:
            setattr(sys.modules['__main__'], args.base, LiveData(**vars(args)))
